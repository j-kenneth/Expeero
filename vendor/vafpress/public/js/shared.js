...
vp.validateURL = function(type, val) {
	...
	// original: var regex = new RegExp(/^(ftp|http|https):\/\/[^ "]+$/i);
...
vp.dependency_event = function(ids, idx, field, func, parent){
	...
	var change    = ['vp-sorter','vp-select', 'vp-checkbox', 'vp-checkimage', 'vp-radiobutton', 'vp-radioimage', 'vp-multiselect', 'vp-toggle', 'vp-upload'],
	    typing    = ['vp-textbox', 'vp-slider', 'vp-color', 'vp-date'],
	...
	{
		jQuery(parent).delegate(name, 'change', function(){vp.binding_action(ids, field, func, 'option');});//thecase
	}
};
