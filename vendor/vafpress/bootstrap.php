...
// Load Languages       //
//////////////////////////
//load_theme_textdomain('vp_textdomain', VP_DIR . '/lang'); as the text used the theme domain
	...
	function vp_ajax_wrapper()
	{
		...
		if( VP_Security::instance()->is_function_whitelisted($function) )
		{
			...
			try {
				...
				$result['status']  = true;
				$result['message'] = esc_html__("Successful", 'adventure-tours');
			} catch (Exception $e) {
				$result['data']    = '';
				$result['status']  = false;
		...
		else
		{
			$result['data']    = '';
			$result['status']  = false;
			$result['message'] = esc_html__("Unauthorized function", 'adventure-tours');		
		}
