...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				'page_title'                      => esc_html__( 'Install Required Plugins', 'adventure-tours' ),
				'menu_title'                      => esc_html__( 'Install Plugins', 'adventure-tours' ),
				'installing'                      => esc_html__( 'Installing Plugin: %s', 'adventure-tours' ),
				'oops'                            => esc_html__( 'Something went wrong.', 'adventure-tours' ),
				...
				'return'                          => esc_html__( 'Return to Required Plugins Installer', 'adventure-tours' ),
				'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'adventure-tours' ),
				'complete'                        => esc_html__( 'All plugins installed and activated successfully. %1$s', 'adventure-tours' ),
			);
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				if ( $this->is_automatic ) {
					...
					if ( is_wp_error( $activate ) ) {
						...
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . esc_html__( 'Return to Required Plugins Installer', 'adventure-tours' ) . '</a></p>';
				...
				foreach ( $this->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . esc_html__( $this->strings['return'], 'adventure-tours' ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . esc_html__( 'Return to the Dashboard', 'adventure-tours' ) . '">' . esc_html__( 'Return to the Dashboard', 'adventure-tours' ) . '</a>' ) . '</p>';
			...
			elseif ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-activate' )] ) && 'activate-plugin' == $_GET[sanitize_key( 'tgmpa-activate' )] ) ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					if ( ! isset( $_POST[sanitize_key( 'action' )] ) ) {
						$msg = sprintf( esc_html__( 'The following plugin was activated successfully: %s.', 'adventure-tours' ), '<strong>' . $plugin['name'] . '</strong>' );
				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters(
						...
						array(
							...
							'dismiss'  => '<a class="dismiss-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . esc_html__( 'Dismiss this notice', 'adventure-tours' ) . '</a>',
						)
					);
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				...
				if ( isset( $plugin['external_url'] ) ) {
					...
					$table_data[$i]['source'] = esc_html__( 'External Link', 'adventure-tours' );
				}
				elseif ( isset( $plugin['source'] ) ) {
					/** The plugin must be from a private repository */
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) )
						$table_data[$i]['source'] = esc_html__( 'Private Repository', 'adventure-tours' );
					/** The plugin is pre-packaged with the theme */
					else
						$table_data[$i]['source'] = esc_html__( 'Pre-Packaged', 'adventure-tours' );
				}
				/** The plugin is from the WordPress repository */
				else {
					$table_data[$i]['source'] = esc_html__( 'WordPress Repository', 'adventure-tours' );
				}

				$table_data[$i]['type'] = $plugin['required'] ? esc_html__( 'Required', 'adventure-tours' ) : esc_html__( 'Recommended', 'adventure-tours' );

				if ( ! isset( $installed_plugins[$plugin['file_path']] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', esc_html__( 'Not Installed', 'adventure-tours' ) );
				elseif ( is_plugin_inactive( $plugin['file_path'] ) )
					$table_data[$i]['status'] = sprintf( '%1$s', esc_html__( 'Installed But Not Activated', 'adventure-tours' ) );
		...
		public function no_items() {
			...
			printf( esc_html__( 'No plugins to install or activate.', 'adventure-tours' ) . ' <a href="%1$s" title="' . esc_attr__( 'Return to the Dashboard', 'adventure-tours' ) . '">' . esc_html__( 'Return to the Dashboard', 'adventure-tours' ) . '</a>', admin_url() );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => esc_html__( 'Plugin', 'adventure-tours' ),
				'source' => esc_html__( 'Source', 'adventure-tours' ),
				'type'   => esc_html__( 'Type', 'adventure-tours' ),
				'status' => esc_html__( 'Status', 'adventure-tours' )
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => esc_html__( 'Install', 'adventure-tours' ),
				'tgmpa-bulk-activate' => esc_html__( 'Activate', 'adventure-tours' ),
			);
	...
	if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
		...
		class TGM_Bulk_Installer extends WP_Upgrader {
			...
			public function install_strings() {
				...
				$this->strings['no_package']          = esc_html__( 'Install package not available.', 'adventure-tours' );
				$this->strings['downloading_package'] = esc_html__( 'Downloading install package from', 'adventure-tours' ) . ' <span class="code">%s</span>&#8230;';
				$this->strings['unpack_package']      = esc_html__( 'Unpacking the package', 'adventure-tours' ) . '&#8230;';
				$this->strings['installing_package']  = esc_html__( 'Installing the plugin', 'adventure-tours' ) . '&#8230;';
				$this->strings['process_failed']      = esc_html__( 'Plugin install failed.', 'adventure-tours' );
				$this->strings['process_success']     = esc_html__( 'Plugin installed successfully.', 'adventure-tours' );
			...
			public function activate_strings() {
				...
				$this->strings['activation_failed']  = esc_html__( 'Plugin activation failed.', 'adventure-tours' );
				$this->strings['activation_success'] = esc_html__( 'Plugin activated successfully.', 'adventure-tours' );
	...
	if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
		...
		class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
			...
			public function add_strings() {
				...
				if ( TGM_Plugin_Activation::$instance->is_automatic ) {
					$this->upgrader->strings['skin_upgrade_start']        = esc_html__( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'adventure-tours' );
					$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed and activated successfully.', 'adventure-tours' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'adventure-tours' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'adventure-tours' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = esc_html__( 'All installations and activations have been completed.', 'adventure-tours' );
					$this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'adventure-tours' );
				}
				/** Default installation strings */
				else {
					$this->upgrader->strings['skin_upgrade_start']        = esc_html__( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'adventure-tours' );
					$this->upgrader->strings['skin_update_failed_error']  = esc_html__( 'An error occurred while installing', 'adventure-tours' ) . ' %1$s: <strong>%2$s</strong>.';
					$this->upgrader->strings['skin_update_failed']        = esc_html__( 'The installation of %1$s failed.', 'adventure-tours' );
					$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'adventure-tours' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html__( 'Show Details', 'adventure-tours' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'adventure-tours' ) . '</span>.</a>';
					$this->upgrader->strings['skin_upgrade_end']          = esc_html__( 'All installations have been completed.', 'adventure-tours' );
					$this->upgrader->strings['skin_before_update_header'] = esc_html__( 'Installing Plugin %1$s (%2$d/%3$d)', 'adventure-tours' );
				}
			...
			public function bulk_footer() {
				...
				foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', TGM_Plugin_Activation::$instance->menu, admin_url( TGM_Plugin_Activation::$instance->parent_url_slug ) ) . '" title="' . esc_attr( TGM_Plugin_Activation::$instance->strings['return'] ) . '" target="_parent">' . esc_html__( TGM_Plugin_Activation::$instance->strings['return'], 'adventure-tours' ) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
				...
				if ( empty( $complete ) ) {
					echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . esc_html__( 'Return to the Dashboard', 'adventure-tours' ) . '">' . esc_html__( 'Return to the Dashboard', 'adventure-tours' ) . '</a>' ) . '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				}
