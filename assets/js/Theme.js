...
Theme.tourBookingForm = {
	...
	_getTimeOptions:function( date ){
		...
		if ( hash && hash[date] ) {
			for(var time in hash[date].times ) {
				result.push({
					val: time,
					text: Theme.formatter.sprintf( this.timeSeletTextFormat, time, hash[date].times[time] )
				});
			}
		}
	...
	_getFormPriceItemsData:function( url, data, refreshCache, callback ){
	...
	_getVariationFor:function( attributes ){
		...
		var list = this.variationsData;
		if ( ! list || list.length < 1 || ! attributes || jQuery.isEmptyObject( attributes ) ) {
			return null;
		}
		var i, cur, is_pass;
		for( i = 0; i<list.length; i++ ) {
			...
			for(var name in attributes) {
				if ( ! cur.attributes[name] || cur.attributes[name] != attributes[name] ) {
					is_pass = false;
					break;
				}
			}
