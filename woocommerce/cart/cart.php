	...
	<table class="shop_table cart" cellspacing="0">
		<thead>
			<tr>
				...
				<th class="product-subtotal"><?php esc_html_e( 'Total', 'adventure-tours' ); ?></th>
			</tr>
		</thead>
		<tbody>
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="product-remove">
							...
							<?php
								echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf( '<a href="%s" class="remove" title="%s">&times;</a>', esc_url( WC()->cart->get_remove_url( $cart_item_key ) ), esc_html__( 'Remove this item', 'adventure-tours' ) ), $cart_item_key );
							?>
						</td>
						...
						<td class="product-name">
							...
							<?php
								...
								if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) )
									echo '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'adventure-tours' ) . '</p>';
							?>
						</td>
						...
						<td class="product-subtotal">
							<span class="cart-responsive-title"><?php esc_html_e( 'Total', 'adventure-tours' ); ?></span>
							<?php
				...
				<td colspan="6" class="actions">
					...
					<?php if ( WC()->cart->coupons_enabled() ) { ?>
						<div class="coupon">
							...
							<label for="coupon_code"><?php esc_html_e( 'Coupon', 'adventure-tours' ); ?>:</label> <input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'adventure-tours' ); ?>" /> <input type="submit" class="button cart-button" name="apply_coupon" value="<?php esc_attr_e( 'Apply Coupon', 'adventure-tours' ); ?>" />
